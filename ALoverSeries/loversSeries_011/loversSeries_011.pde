TrailParticle[] trail;
boolean doLights,doOrtho,doFirst,doAlpha,doRumble;
...
void draw(){
  ...
  translate((doFirst?-trail[0].pos.x:0),-trail[0].pos.y-height/4,0);
...
void keyPressed(){
  ...
  if(key=='f'){doFirst=!doFirst;}
  if(key=='a'){doAlpha=!doAlpha;}
  if(key=='r'){doRumble=!doRumble;}
}
void doLights(){
