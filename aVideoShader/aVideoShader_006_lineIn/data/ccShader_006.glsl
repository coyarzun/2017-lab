...
void main(void) {
    ...
    //vec4 color2      = color;//texture2D(texture, pp);
    vec3 aux = rgb2hsv(vec3(color.r,color.g,color.b));
    aux.x += time;
    aux.y += 2.0*time;// 1.0;//+= time;//1.0;
    aux.z += 4.0*time; //= 1.0;
    vec4 color2      = color;
    vec3 aux2 = hsv2rgb(aux);
    color2.r = aux2.r;
    color2.g = aux2.g;
    color2.b = aux2.b;

    //color;//texture2D(texture, pp);
    ...
    color.r         = texture2D(textureMap, vec2(color2.r, 0)).r;
    color.g         = texture2D(textureMap, vec2(color2.g, 0)).g;
    color.b         = texture2D(textureMap, vec2(color2.b, 0)).b;
