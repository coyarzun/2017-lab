...
void setup(){
  ...
  if(!true)
  pattern = loadImage("cracked_ground.jpg");
  else
...
void draw(){
  background(0);
    {
    pattern = createImage(100,100,RGB);//
    pattern.loadPixels();
...
int[][] calculateLinksForRow(int dwidth, int y, PImage depthData) {
    ...
    for(int x = 0; x < dwidth; x++) {
        float separation = getSeparation(getIndex(x,y,dwidth), depthData);
...
void generate(PImage pattern, PImage depthmap) {
    ...
    for(int y = 0; y < depthmap.height; y++) {
        ...
        for(int x = 0; x < dwidth; x++) {
            ...
            set(x, y, color(
            pattern.pixels[getIndex(kolor[x]%pattern.width,y%pattern.height,pattern.width)]
            //depthmap.pixels[getIndex(x,y,width)] = (depthmap.pixels[x+y*width])
            ) 
