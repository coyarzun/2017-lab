...
void draw() {
  ...
  translate(- unit*cols/2, - unit*rows/2, -width/2);
  //rotateX(-radians(15+1*75));
  //rotateY(radians(45));
  doLights();
  pushMatrix();
  ...
  for (int i=0; i<cols; i++) {
    for (int j=0; j<rows; j++) {
      pushMatrix();
      translate(i*unit, j*unit, 0);//-j*unit//+j*unit
