...
void setup(){
  size(800,400,P3D);
  cols = 8*2; rows = 8*5; zcols = 8*2;
  walker = new ThreeDeeWalker();
}
void draw(){
  if(doScaleUnit)unit = 16*4;else unit = 32;
  background(0);
  camera();
...
void doView(){
  if(wichView==0){
    ...
    rotateX(walker.paths.get(walker.paths.size()-1).direction.x*PI/2);
    rotateY(walker.paths.get(walker.paths.size()-1).direction.y*PI/2);
    rotateZ(walker.paths.get(walker.paths.size()-1).direction.z*PI/2);
    /*
